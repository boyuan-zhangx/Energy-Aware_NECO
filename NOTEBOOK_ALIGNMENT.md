...
- Default checkpoints/history: `checkpoints/unet.pth`, `checkpoints/unet_ens_*.pth`, and `results/training_history.json` are synchronized from the original notebook artifacts. The previous translated-project artifacts are preserved under `checkpoints/pre_alignment/` and `results/pre_alignment/`.
